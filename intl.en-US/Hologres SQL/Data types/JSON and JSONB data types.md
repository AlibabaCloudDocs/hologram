...
|row\_to\_json\(record \[, pretty\_bool\]\)|Returns a row as a JSON object. If the value of the pretty\_bool parameter is true, line feeds are added between dimension-1 elements.|`select row_to_json(row(1,'foo'))`|`{"f1":1,"f2":"foo"}`|
