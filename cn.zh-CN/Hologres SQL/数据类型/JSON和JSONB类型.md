...
|json\_object\(text\[\]\)|此函数可以从一个文本数组构造一个JSON对象。该数组必须可以是具有偶数个成员的一维数组（成员被当做交替出现的键/值对），或者是一个二维数组（每一个内部数组刚好有2个元素，可以被看做是键/值对）。|`select json_object('{a, 1, b, "def", c, 3.5}');`|`{"a": "1", "b": "def", "c": "3.5"}`|
|jsonb\_object\(text\[\]\)|`select jsonb_object('{a, 1, b, "def", c, 3.5}');`|`{"a": "1", "b": "def", "c": "3.5"}`|
|json\_object\(keys text\[\], values text\[\]\)|json\_object的这种形式从两个独立的数组得到键/值对。在其他方面和一个参数的形式相同。|`select json_object('{a, b}', '{1,2}')`|`{"a": "1", "b": "2"}`|
