...
2.  设置数据脱敏。
    ...
    Hologres支持对目标列或目标用户设置数据脱敏。您可以通过指定具体列名或用户类型，并使用系统预设的脱敏函数进行脱敏。Hologres支持的脱敏函数，详情请参见[t1962205.md\#section\_lbl\_y6k\_kjx]()。
    ...
    -   对用户或用户组脱敏。
        ...
        |--|--|
        |hg\_anon|hg\_anon是Hologres内部封装的扩展函数，您需要调用该扩展函数才能开启数据脱敏功能。|
        |<user\_name\>|user\_name可以为用户或用户组名称。如果需要对多个用户进行相同的脱敏设置，则可以将用户先添加到用户组，再对用户组进行脱敏。|
