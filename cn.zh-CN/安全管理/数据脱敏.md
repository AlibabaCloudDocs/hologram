...
4.  对用户进行脱敏。
    ...
    可由Superuser对目标用户进行脱敏，具体命令、示例与参数说明如下。
    ...
    -   参数说明：
        ...
        |--|--|
        |hg\_anon|hg\_anon是Hologres内部封装的扩展函数，您需要调用该扩展函数才能开启数据脱敏功能。|
        |<user\_name\>|user\_name可以为用户名称。使用时需要替换<user\_name\>为实际用户名称。|
...
指系统预设的脱敏函数，详情请参见[\#section\_lbl\_y6k\_kjx](#section_lbl_y6k_kjx)。使用label\_name，则表示对目标用户使用label\_name脱敏的列，以指定形式显示。
