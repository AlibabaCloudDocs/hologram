...
您也可以使用`[IMPORT FOREIGN SCHEMA](/intl.zh-CN/SQL参考/DDL/SCHEMA/IMPORT FOREIGN SCHEMA.md)`语句批量创建外部表。也可以将该命令语句在数据开发中执行，并前往调度，实现MaxCompute表新增后Hologres外表也自动新增，详情见[t1501233.md\#](/intl.zh-CN/基于HoloStudio的开发/数据开发/Hologres开发：周期性调度.md)。
...
5.  在**PG管理**页面，查看批量创建的外部表并查询表数据，详情请参见[创建及管理外部表](/intl.zh-CN/基于HoloStudio的开发/PG管理/外部表.md)。
